...
public class MainActivity extends AppCompatActivity {
    ...
    private void createDatabase(String name) {
        ...
        //openOrCreateDatabase 메소드는 DB를 생성하거나 영어주는 메소드
        // 1번째 패러미터는 데이터베이스 이름
        // 2번째 패러미터는 사용모드 MODE_PRIVATE,
        // MODE_READABLE, MODE_WORLD_WAITABLE : 다른 애플리케이션에서도
        // 이 데이터베이스에 대한 접근을 허용하겠다는 의미
        // 3번째패러미터는 선택적 지점. null이 아닌 객체를 지정할 경우에는
        // 쿼리의 결과값으로 리턴되는 커서를 만들 객체가 전달된다.
       try {
